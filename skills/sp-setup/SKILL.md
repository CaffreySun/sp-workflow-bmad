---
name: "sp-setup"
description: Sets up Superpowers Workflow module in a project. Use when the user requests to 'install sp module', 'configure Superpowers Workflow', or 'setup Superpowers Workflow'.
---
...
3. Check for per-module configuration at `{project-root}/_bmad/sp/config.yaml` and `{project-root}/_bmad/core/config.yaml`. If either file exists:
...
```bash
python3 ./scripts/cleanup-legacy.py --bmad-dir "{project-root}/_bmad" --module-code sp --also-remove _config --skills-dir "{project-root}/.claude/skills"
```
